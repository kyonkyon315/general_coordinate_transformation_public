...
int main()
{
    ...
    Value vx_square_av = 0.;
    Value vy_square_av = 0.;
    Value vz_square_av = 0.;
    ...
    for(int world_rank = 0; world_rank < n_proc; ++world_rank){
        ...
        const Physic_vx operator_vx(world_rank);
        const Physic_vy operator_vy(world_rank);
        const Physic_vz operator_vz(world_rank);
            ...
            for (int vp_local = vp_start; vp_local < vp_end; ++vp_local) {
                for (int vr_local = vr_start; vr_local < vr_end; ++vr_local) {
                    for (int vt_local = vt_start; vt_local < vt_end; ++vt_local) {
                        const Value vx =  operator_vx.translate(z_local,vr_start, vt_local, vp_local);
                        const Value vy =  operator_vy.translate(z_local,vr_start, vt_local, vp_local);
                        const Value vz =  operator_vz.translate(z_local,vr_start, vt_local, vp_local);
                        const Value f = data.at(z_local,vr_start, vt_local, vp_local);
                        vx_square_av += (f*vx*vx);
                        vy_square_av += (f*vy*vy);
                        vz_square_av += (f*vz*vz);
                    }
                }
            }
        }
    ...
    std::cout<<"moment of vx^2:"<<std::setprecision(16)<<vx_square_av<<"\n";
    std::cout<<"moment of vy^2:"<<std::setprecision(16)<<vy_square_av<<"\n";
    std::cout<<"moment of vz^2:"<<std::setprecision(16)<<vz_square_av<<"\n";

    std::cout<<"anisotropy:"<<std::setprecision(16)<<vz_square_av/vx_square_av<<"\n";

...
public:
    ...
    Value apply(const int calc_vr)const{
        //r = xi + 0.5 + arctan() 
        /*
        def f(x):
            a=10.
            b=1.
            y = x+0.5 + (a-1)*b* np.arctan((x+0.5)/b)
            return y
        */
        const double global_calc_r = (double)(vr_start_id + calc_vr);
        return Global::grid_size_vr * (global_calc_r + 0.5 + (a-1.)*b* std::atan((global_calc_r + 0.5)/b));
    }

    Value dr_dcalc(const int calc_vr)const{
        const double global_calc_r = (double)(vr_start_id + calc_vr);
        return Global::grid_size_vr * (1. + (a-1.)/(1. + (global_calc_r+0.5)*(global_calc_r+0.5)/(b*b)));
    }
    Value dcalc_dr(const int calc_vr)const{
        return 1./dr_dcalc(calc_vr);
    }
};
...
public:
    Value honestly_translate(const int calc_vr,const int calc_vt,const int calc_vp){
        // sinθ cosφ
        const Value dcalcr_dr = calc_vr_2_vr.dcalc_dr(calc_vr);
        const Value vt = calc_vt_2_vt.apply(calc_vt);
        const Value vp = calc_vp_2_vp.apply(calc_vp);
        return dcalcr_dr * sin(vt) * cos(vp);
    }
...
{
private:
    //dcalcr_dr , sin(vt) , sin(vp)を個別に事前計算して、最後の掛け算は毎回やったほうがいいかも。メモリの節約のために
    NdTensorWithGhostCell<Value,Axis_vr,Axis_vt,Axis_vp> table;
    const CalcVt_2_Vt calc_vt_2_vt;
    const CalcVp_2_Vp calc_vp_2_vp;
    const CalcVr_2_Vr calc_vr_2_vr;
public:
    Value honestly_translate(const int calc_vr,const int calc_vt,const int calc_vp){
        ...
        const Value vt = calc_vt_2_vt.apply(calc_vt);
        const Value vp = calc_vp_2_vp.apply(calc_vp);
        return dcalcr_dr * sin(vt) * sin(vp);
    }
...
private:
    ...
    Value honestly_translate(const int calc_vr,const int calc_vt){
        ...
        const Value vr = calc_vr_2_vr.apply(calc_vr);
        const Value vt = calc_vt_2_vt.apply(calc_vt);
        return (vr*vr*sin(vt)) *calc_vr_2_vr.dr_dcalc(calc_vr)
                *Global::grid_size_z_*Global::grid_size_vt*Global::grid_size_vp;
    }
public:
...
bool is_vt_left_edge(const int my_world_rank){
    auto [axis_z_, axis_vr, axis_vt, axis_vp] = axis_instantiator<Axis_z_,Axis_vr,Axis_vt,Axis_vp>(my_world_rank);
    return axis_vt.block_id == 0;
}

bool is_vt_right_edge(const int my_world_rank){
    auto [axis_z_, axis_vr, axis_vt, axis_vp] = axis_instantiator<Axis_z_,Axis_vr,Axis_vt,Axis_vp>(my_world_rank);
    return axis_vt.block_id == Axis_vt::num_blocks-1;
}
...
private:
    ...
    const bool _is_vt_left_edge;
    const bool _is_vt_right_edge;
...
public:
    Fvx(const int my_world_rank,
        ...
        _is_vt_left_edge(is_vt_left_edge(my_world_rank)),
        _is_vt_right_edge(is_vt_right_edge(my_world_rank)),
        e_field(e_field),
        m_field(m_field),
        physic_vz(physic_vz),
        physic_vy(physic_vy)
    ...
    Value at(const int calc_z,const int calc_vr,const int calc_vt,const int calc_vp) const {
        ...
        if(_is_velo_right_edge && calc_vr == Axis_vr::num_grid){
            ...
            const Value retval = - (Ex + vy*Bz - vz*By);//電子の電荷が負なので - がつく
            //規格化したので移流項はExのみ

            if(
                (_is_vt_left_edge && calc_vt == -1) || 
                (_is_vt_right_edge && calc_vt == Axis_vt::num_grid)||
                (_is_velo_left_edge && calc_vr == -1))
            {//反対側にいくと軸の正の向きが逆になるのでそれを戻す
                return - retval;
            }
            else{
                return retval;
...
public:
    ...
    Value at(const int calc_z,const int calc_vr,const int calc_vt,const int calc_vp) const {
        ...
        if(_is_velo_right_edge && calc_vr == Axis_vr::num_grid){
            ...
            const Value retval = - (Ey + vz*Bx - vx*Bz);//電子の電荷が負なので - がつく
            if(
                (_is_vt_left_edge && calc_vt == -1) || 
                (_is_vt_right_edge && calc_vt == Axis_vt::num_grid)||
                (_is_velo_left_edge && calc_vr == -1))
...
public:
    ...
    Value at(const int calc_z,const int calc_vr,const int calc_vt,const int calc_vp) const {
        ...
        if(_is_velo_right_edge && calc_vr == Axis_vr::num_grid){
            ...
            const Value retval = - (Ez + vx*By - vy*Bx);//電子の電荷が負なので - がつく
            //規格化したので移流項はExのみ
...
Value fM(const Value vx_tilde,const Value vy_tilde,const Value vz_tilde/*無次元量が入る*/){
    ...
    const Value v_th_para = Global::v_th_para;
    const Value v_th_perp = 1.;

    return Norm::Coef::Ne_tilde 
            * std::exp(
                -v_para * v_para /(2.*v_th_para * v_th_para) 
                    - v_perp * v_perp /(2.*v_th_perp * v_th_perp)
            )/(v_th_para*v_th_perp*v_th_perp * Utils::ConstExpr::sqrt(2.*M_PI)*(2.* M_PI));
...
void init_dist_and_poisson(const int my_world_rank,DistributionFunction& f,ElectricField& e_field){
    ...
    CalcVr_2_Vr calc_vr_2_vr_global(0);
    ...
    for(int j=0;j<Axis_vr::num_global_grid;++j){
        for(int k=0;k<Axis_vt::num_global_grid;++k){
            for(int l=0;l<Axis_vp::num_global_grid;++l){
                ...
                Value vr = calc_vr_2_vr_global.apply(j);
                ...
                Value vz = vr * cos(vt) ;
                    ...
                    n += Global::grid_size_z_*calc_vr_2_vr_global.dr_dcalc(j)
                        *Global::grid_size_vt*Global::grid_size_vp
                        *vr*vr*sin(vt)
                    *fM(vx,vy,vz);
            }
        }
...
int main(int argc, char** argv)
{
    ...
    if(axis_vr.block_id==0 && axis_vt.block_id==0 && axis_vp.block_id==0){
        std::string strage_path = "/LARGE1/gr20001/b39211/Documents/general_coodinate_transformation/whistler";
    ...
    if(world_rank==0)std::cout<<"d"<<std::endl;

    for(int i=0;i<num_steps;i++){
        if(world_rank==0 && i%100==0)std::cout<<i<<" current:"<<current.at(0).x<<","<<current.at(0).y<<","<<current.at(0).z<<std::endl;

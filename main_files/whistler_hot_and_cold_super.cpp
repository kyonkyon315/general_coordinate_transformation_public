...
Value fM(const Value vx_tilde,const Value vy_tilde,const Value vz_tilde/*無次元量が入る*/){
    ...
    const Value v_th_perp_hot = 1.;
    const Value v_th_para_hot = Global::v_th_para;

    const Value v_th_cold = 0.2;//=0.01c

    const Value hot_ratio = 0.01;

    const Value f_hot = hot_ratio * Norm::Coef::Ne_tilde 
            * std::exp(
                -v_para * v_para /(2.*v_th_para_hot * v_th_para_hot) 
                    - v_perp * v_perp /(2.*v_th_perp_hot * v_th_perp_hot)
            )/(v_th_para_hot * v_th_perp_hot * v_th_perp_hot * Utils::ConstExpr::sqrt(2.*M_PI)*(2.* M_PI));
    //Ne_tilde = int f_tilde dv_tilde^3

    const Value f_cold = (1.-hot_ratio) * Norm::Coef::Ne_tilde 
            * std::exp(
                -v_para * v_para /(2.*v_th_cold * v_th_cold) 
                    - v_perp * v_perp /(2.*v_th_cold * v_th_cold)
            )/(v_th_cold * v_th_cold * v_th_cold * Utils::ConstExpr::sqrt(2.*M_PI)*(2.* M_PI));
...
int main(int argc, char** argv)
{
    ...
    int num_steps = 65000;
    if(world_rank==0)std::cout<<"whistler_hot_and_cold_super.cpp\n";
